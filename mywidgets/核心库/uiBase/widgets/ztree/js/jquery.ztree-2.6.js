...
(function($) {
	...
	function appendTreeNodes(setting, level, treeNodes, parentNode) {
		...
		for (var i = 0, l = treeNodes.length; i < l; i++) {
			var node = treeNodes[i];
			//add by zhangwei 2011-7-29 解决异步加载子节点，如果父节点被选择，子节点不选中问题
	...
	function makeNodeIcoStyle(setting, treeNode) {
		var icoStyle = [];
		if (!treeNode.isAjaxing) {
		    //add by zhangwei 2011-7-26 解决树实现类不设置打开关闭图标时，图标错误的问题
		    if(!(treeNode.iconOpen && treeNode.iconClose)){
		        if(treeNode.isParent){
		            treeNode.iconOpen = CTX+"/uiBase/widgets/ztree/img/folder_Open.gif";
		            treeNode.iconClose =CTX+"/uiBase/widgets/ztree/img/folder_Close.gif";
		        }else{
		            treeNode.iconOpen = CTX+"/uiBase/widgets/ztree/img/page.gif";
		            treeNode.iconClose =CTX+"/uiBase/widgets/ztree/img/page.gif";
		        }
			} 
			var icon = (treeNode.iconOpen && treeNode.iconClose) ? (treeNode.open ? treeNode.iconOpen : treeNode.iconClose) : treeNode.icon;
	...
	function setParentNodeCheckBox(setting, treeNode, value) {
		...
		if (treeNode.parentNode) {
		    //update by zhangwei 2011-7-29 修正只要有一个子节点不选中，则其父节点则就不选中
//			var pSign = true;
//			if (!value) {
...
//			}
    ...
    //遍历子节点展开 或 折叠,支持异步加载 add by zhangwei
	function expandCollapse(setting, treeNode, expandSign, animateSign, callback) {
		var treeNodes = (treeNode) ? treeNode[setting.nodesCol]: setting.root[setting.nodesCol];
		//针对动画进行优化,一般来说只有在第一层的时候，才进行动画效果
		var selfAnimateSign = (treeNode) ? false : animateSign;
	    if(!treeNodes&&treeNode&&treeNode.isParent&&!treeNode[setting.nodesCol]&&setting.async){
            asyncGetNodeForExpandAll(setting, treeNode);
            var node = zTree.getNodeByTId(treeNode.tId);
            treeNodes = node[setting.nodesCol];
		}
		if (treeNodes) {
			for (var i = 0, l = treeNodes.length; i < l; i++) {
				if (treeNodes[i]) expandCollapse(setting, treeNodes[i], expandSign, selfAnimateSign);
			}
		}
		//保证callback只执行一次
		expandAndCollapseNode(setting, treeNode, expandSign, animateSign, callback );
	}
	//同步提交，解决expandAll方法不能全部打开的问题 add by zhangwei
	function asyncGetNodeForExpandAll(setting, treeNode) {
		var i, l;
		if (treeNode && (treeNode.isAjaxing || !treeNode.isParent)) {
			return;
		}
	...
	function selectManyNode(setting,treeNode){
		$("#" + treeNode.tId + IDMark_A).addClass(Class_CurSelectedNode);
		/*desc 树定位到查询到的结果 by gaona at 2012-04-19 begin*/
		var el = treeNode.tId ;
		
		/* desc 当自定义Tree 唯一标识，主UL的ID（treeObjId）的时候，这里写死的 #treeDemo 定位方式就会出错 begin*/
		// var scrollTar = $('#'+el).parents('#treeDemo').parent('div') ;
		var scrollTar = $('#'+el).parents('#'+setting.treeObjId).parent('div') ;
		/* desc 当自定义Tree 唯一标识，主UL的ID（treeObjId）的时候，这里写死的 treeDemo 定位方式就会出错 end*/
		
		var t = $(scrollTar).scrollTop() ;
		if(t!=0){
			$(scrollTar).scrollTop(0) ;
		}
		var elTop = $('#'+el).offset().top ;
		var elParTop = $(scrollTar).offset().top ;
		var devTop = elTop - elParTop ;
		var scrT = parseInt(devTop) ;
		setTimeout(function(){
			$(scrollTar).scrollTop(scrT); 
		},100) ;
		/*desc 树定位到查询到的结果 by gaona at 2012-04-19 end*/
	}
	
	function cancalManyNode(){
		var selectDom = $("."+Class_CurSelectedNode);
		selectDom.removeClass("curSelectedNode");
	}
	...
	function getTreeOpenNodes(setting, treeNodes){
	    if (!treeNodes) return [];
	    var results = [];
	    alert("treeNodes.length="+treeNodes.length);
	    for (var i = 0, l = treeNodes.length; i < l; i++) {
	        alert(treeNodes[i].name);
			if (treeNodes[i].open==true) {
				results = results.concat([treeNodes[i]]);
			}
			var tmp = getTreeOpenNodes(setting, treeNodes[i][setting.nodesCol]);
			if (tmp.length > 0) results = results.concat(tmp);
		}
		return results;
	}
	...
	function zTreePlugin(){
		return {
			...
			getTreeOpenNodes:function(){
			    return getTreeOpenNodes(this.setting, this.setting.root[this.setting.nodesCol]);
			},
			...
			expandAll : function(expandSign) {
				expandCollapse(this.setting, null, expandSign, true);
			},
...
//					}
			 ...
			 selectManyNode : function(treeNode){
			 	if (!treeNode) return;

				if (st.checkEvent(this.setting)) {
					selectManyNode(this.setting, treeNode);
					//如果选择某节点，则必须展开其全部父节点
					//多个图层同时进行动画，导致产生的延迟很难用代码准确捕获动画最终结束时间
					//因此为了保证准确将节点focus进行定位，则对于js操作节点时，不进行动画
//					if (treeNode.parentNode) {
