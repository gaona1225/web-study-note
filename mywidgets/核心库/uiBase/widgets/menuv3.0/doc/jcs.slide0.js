...
/**
 * @(应用-添加应用 一级菜单滑动能力检测)
 * @param menuFlag( 0 应用 1 添加应用)--->移植到容器自定义属性中data-topScrollFlag(0:应用(默认);1-添加应用)
 */
function _appScrollListener(){
	var liHeight = 26;
	var maxNum = 13;
	var $topMenuCon = $('#allOneMenuUL');
	var menuFlag = $topMenuCon.attr('data-topScrollFlag');
	menuFlag = (menuFlag == undefined ||menuFlag>1)?0:menuFlag;
	...
	if(downNum > 0){
		$("#fw_d").css({
    		...
    		"position":"absolute",
    		"left":left,
    		"top":outerConHeight+((menuFlag == 0)?11:21)
    	});		
	}else{
...
function _ui5scrllMe(e){
    ...
    */
    var scrllFlag = _appScrollListener();
    var liHeight = 26;
    var $ulCon = $("#ulOuterCon");
	var $ul = $ulCon.find("ul");
...
function mouseSlide(){
	...
	addEventHandler(ulOuterCon,mousewheelevt,_ui5scrllMe);
	//addEventHandler(ulOuterCon,mousewheelevt,function(e,menuFlag){
	//	alert(menuFlag);
	//});
};
