...
$.formValidator = 
{
	...
	{	
		...
		else
		{
			...
			{	
				var html;
				if(2*$(window).width()/3>$(elem).offset().left+$(elem).width()){
					html = showclass == "onShow" ? onShowHtml : (showclass == "onFocus" ? onFocusHtml : (showclass == "onCorrect" ? onCorrectHtml : onErrorHtml));
				}else if($(window).width()-$(elem).offset().left < 130){
					html = showclass == "onShow" ? onShowHtml : (showclass == "onFocus" ? onFocusHtml2 : (showclass == "onCorrect" ? onCorrectHtml : onErrorHtml2));
				}else{
					html = showclass == "onShow" ? onShowHtml : (showclass == "onFocus" ? onFocusHtml1 : (showclass == "onCorrect" ? onCorrectHtml : onErrorHtml1));
				}
	...
	showMessage : function(returnObj)
	{
		...
		{	
			//验证成功后,如果没有设置成功提示信息,则给出默认提示,否则给出自定义提示;允许为空,值为空的提示;移除表单验证class
		...
		switch(sType)
		{
			...
			case "file":
				if (setting.type == "size") {
					empty = setting.empty;
					var inputTextDomObjs = $('input[type="text"]').get(0);
					if(!empty.leftEmpty){
	...
	reloadAutoTip : function(validatorGroup)
	{
		...
		$.each(initConfig.validObjects,function()
		{
			if(initConfig.mode == "AutoTip")
			{
				...
				var y = offset.top;
				var x = $(relativeID ).width() + offset.left + windowWidth;
	...
	return this.each(function(e)
	{
		...
		{	
			...
			if(initConfig.mode == "AutoTip" && tipDiv.length==0)
			{
				...
				var offset = $("#"+relativeID ).offset();
				var windowWidth = $(window).width();
				//如果控件在页面靠左，则提示信息显示在控件右边
				if(2*windowWidth/3 > offset.left+$("#"+relativeID ).width()){
				...
				}else if(windowWidth - offset.left < 130){
					//控件居右且距离页面右边的宽度小于提示信息的宽度，那么提示信息在控件下面靠左显示
					setting_temp.tipCss.top = offset.top + $("#"+relativeID ).height() + setting_temp.tipCss.top;
					setting_temp.tipCss.left = offset.left - $("#"+relativeID ).width()/2;
				}else{
					//控件居右且距离页面右边的宽度大于提示信息的宽度，那么提示信息在控件下面靠右显示
					setting_temp.tipCss.top = offset.top + $("#"+relativeID ).height() + setting_temp.tipCss.top;
					setting_temp.tipCss.left = offset.left + setting_temp.tipCss.left;
				}
				//width:100%使提示框随提示信息的长度自适应，但width就是body的宽度，会出现滚动条
				var formValidateTip = $("<div style='position:absolute;width:100%;' id='"+tip+"'></div>").appendTo($("body"));
				//设置提示框div的宽度为页面宽度减去提示框距离页面左边的宽度，这样就可以消去滚动条
				var tipwidth = $('body').width() - setting_temp.tipCss.left; 
				formValidateTip.css(setting_temp.tipCss).css('width',tipwidth);
		...
		if(srcTag == "input" || srcTag=="textarea")
		{
			...
			{	
				...
				if(jqobj.val()==null||jqobj.val()==""){
					...
					if(this.validatorPasswordIndex > 0){
						$("#"+setting.pwdTipID).show();jqobj.trigger('keyup');}
				}else{
					$("#"+tip).hide();//日历控件选择日期后隐藏提示信息
					jqobj.removeClass("validatebox-invalid");
				}
			});
			//按键时候触发校验
			...
			jqobj.bind(setting.triggerEvent, function(){//setting.triggerEvent == 'movermove'
...
$.fn.showTooltips = function()
{
	...
	return this.each(function()
	{
		...
		this.tooltip = $("<span class='fv_tooltip' style='display:block;border:1px solid #CC9933;background:#FFFFCC;padding:3px;'></span>").append(s).append(b).css({"filter":"alpha(opacity:95)","KHTMLOpacity":"0.95","MozOpacity":"0.95","opacity":"0.95"});
		//注册事件
		jqobj.bind({
...
var formValidator_setting = 
{
	...
	triggerEvent:"keyup",/**mousemove根据系统需求，将form验证改成与easyui验证一样，所以需要将其改成keyup事件即可   huanghui @2011-12-29**/
	forceValid : false,
	tipID : null,
	pwdTipID : null,
	fixTipID : null,
