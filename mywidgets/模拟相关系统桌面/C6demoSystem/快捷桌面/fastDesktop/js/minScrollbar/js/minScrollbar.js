    ...
    */
	...
	/** @lends minScrollbar.prototype */
	{
		 options:{
			 ...
			 bgColor:'#000' ,  //滚动条的背景色
			 viewHeight:'100px',  //可视区域的高度值，默认为200px
			 viewWidth:'100px'  //可视区域的高度值，默认为500px
		 },
		 _create:function(){
			 var o = this.options;
		...
		_eventListener:function($target,$hScroll,$vScroll,o){
			...
			if(hasTouch){
				...
				$target[0].ontouchstart = function(e){_start(e);};	
				$target[0].ontouchmove = function(e){_move(e);};	
				$target[0].ontouchend = function(e){_end(e);};	
				$target[0].ontouchcancel = function(e){_end(e);};		
			}else{
		...
		destroy:function(){
			if(this.element.attr('data-destroy')=='true'){
				if('ontouchstart' in window){
					...
					$(window).unbind('mouseup').unbind('mousemove');
