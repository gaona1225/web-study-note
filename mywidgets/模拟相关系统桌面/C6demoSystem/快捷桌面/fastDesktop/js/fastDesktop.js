...
$(function(){
	...
	$.getJSON('../js/list.json',function(data){
		...
		$('.desktopList').sortable({ //jqury 排序
			containment:region, //约束排序动作只能在一个指定的范围内发生。可选值：DOM对象, 'parent', 'document', 'window', 或jQuery对象
			scroll:false, //如果设置成true，则元素被拖动到页面边缘时，会自动滚动。
			cursorAt:{top:25,left:25} , //当开始移动时，鼠标定位在的某个位置上（最多两个方向）。可选值：{ top, left, right, bottom }.
			distance:5, //决定至少要在元素上面拖动多少像素后，才正式触发排序动作。
			...
			remove:function(event,ui){ //当一个元素拖拽移出list对象移出并进入另一个list对象后触发此事件。   
		...
		$('.desktopNav li:not(".pointLi")').draggable({ //jquery 鼠标移动对象
			containment:region ,
			distance:5,
			revert:true ,
			drag:function(event,ui){ //当一个元素移动触发此事件。
				$('.trashbin').css('visibility','visible') ;
			},
			stop:function(event,ui){ //拖动结束触发
				$('.trashbin').css('visibility','hidden') ;
			},
		});
		/*------图标拖拽排序功能结束------*/
