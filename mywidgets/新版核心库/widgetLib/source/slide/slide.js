    ...
    */
	...
	{		
		...
		_create:function(){
			...
			$(_self).find('.slideElem').width(op.width).height(op.height) ;
			var $btnHtml = '<a href="javascript:;" class="btn_prev"></a><a href="javascript:;" class="btn_next"></a>' ;
			$(_self).parents('.slide').append($btnHtml) ;
			...
			if(op.showNum){ //显示切换焦点
				$(_self).parents('.slideWrap').append($numHtml) ;
			}
			op.fn() ;
		},
		_init:function(){
			var _self = this.element ;
			var _selfWrap = $(_self).parents('.slideWrap') ;
			var op = this.options ;			
			...
			switch(op.effect){
				...
				case 'slide-y' : $(_self).nextAll('.btn_prev').addClass('btn_up');$(_self).nextAll('.btn_next').addClass('btn_down'); initPos(0,slideHei) ; break ;
			}
			...
			if(op.showBtn){
				$(_selfWrap).hover(function(){
					$(_self).nextAll('.btn_prev,.btn_next').fadeIn() ;
				},function(){
					$(_self).nextAll('.btn_prev,.btn_next').fadeOut() ;
				}) ;
			}else{				
				$(_self).nextAll('.btn_prev,.btn_next').css('visibility','hidden') ;				
			}
			$(_selfWrap).hover(function(){
				if(op.autoSlide){
			...
			$(_self).nextAll('.btn_prev').click(function(){ //向前切换
				switch(op.effect){
			...
			$(_self).nextAll('.btn_next').click(function(){
				switch(op.effect){
					case 'fade' : slideFade(1) ; break ;
					case 'slide-x' : slideX(1) ; break ;
					case 'slide-y' : slideY(1) ; break ;
			...
			function autoPlay(){
				$(_self).nextAll('.btn_next').click() ;
			}
			//点击切换焦点切换
			$(_selfWrap).find('.num_inner a').click(function(){
				var lastIndex = parseInt($(_selfWrap).find('a.num_on').index()) ;
				var nowIndex = parseInt($(this).index()) ;
				if(lastIndex>nowIndex){
					for(var i= lastIndex-nowIndex - 1; i>=0;i--){
						$(_self).nextAll('.btn_prev').click() ;
					}
			...
			function slideFade(dir){
				...
				$(_selfWrap).find('.num_inner a').eq(fadeCount).addClass('num_on').siblings('a').removeClass('num_on') ;
			}
			//x轴方向滑动方法体
			function slideX(dir){
			...
			function setCur(){
				for(var i = slideLen-1; i>=0; i--){
					if(parseInt($slideElem.eq(i).attr('data-left')) == 0 && parseInt($slideElem.eq(i).attr('data-top')) == 0){
						$(_selfWrap).find('.num_inner a').eq(i).addClass('num_on') ;
					}else{
						$(_selfWrap).find('.num_inner a').eq(i).removeClass('num_on') ;
					}
				}
			}
		},
		...
		*/
		destroy : function(){
			...
			if(_self.parents('div.slideWrap').length>0){
				...
				$(_self).nextAll('.btn_prev,.btn_next').remove() ;
